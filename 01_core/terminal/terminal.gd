...
@onready var input_line: LineEdit = $HBoxContainer/MarginContainer/VBoxContainer/InputLine
@onready var auto_complete_suggestions: RichTextLabel = $HBoxContainer/MarginContainer/VBoxContainer/AutoComplete
@onready var command_parameter: RichTextLabel = $HBoxContainer/MarginContainer/VBoxContainer/CommandParameters
@onready var output: RichTextLabel = $HBoxContainer/Output
...
const COLOR_PRIMARY: Color = Color("#282828")
const COLOR_SECONDARY: Color = Color("#ebdbb2")
const COLOR_TERTIARY: Color = Color("#ffbf00")
...
func _ready() -> void:
	...
	command_executer.register_command_set(DefaultCommands.new(self, output))
...
func _on_input_line_submitted(text: String) -> void:
	...
	command_executer.execute(command, args, output)
	
	input_line.clear()
	_update_autocomplete_suggestions("")
	
	var max_lines = 50
	var current_lines = output.get_line_count()
	
	if current_lines > max_lines:
		var lines = output.text.split("\n")
		var kept_lines = lines.slice(-max_lines)
		output.text = "\n".join(kept_lines)
...
func _apply_styling() -> void:
	...
	if input_line:
		input_line.add_theme_color_override("font_color", COLOR_SECONDARY)
		input_line.add_theme_color_override("caret_color", COLOR_TERTIARY)
		auto_complete_suggestions.add_theme_color_override("default_color", COLOR_SECONDARY)
		command_parameter.add_theme_color_override("default_color", COLOR_SECONDARY)
		
		output.add_theme_color_override("default_color", COLOR_SECONDARY)
		...
		style.border_color = COLOR_SECONDARY
		...
		input_line.add_theme_color_override("selection_color", COLOR_TERTIARY)
		
		var rtl_style = StyleBoxFlat.new()
		rtl_style.bg_color = COLOR_PRIMARY
