...
func execute(command_name: String, args: Array, output: RichTextLabel) -> void:
	...
	for command_set in command_sets:
		...
		if commands.has(command_name): # searching for command
			...
			if cmd.callback.is_valid():
				...
				output.text += "Error: Invalid command callback"
				command_executed.emit(command_name, false)
			return
	
	print("Error: Unknown command " + command_name)
	output.text += "Error: Unknown command " + command_name
	command_executed.emit(command_name, false)
